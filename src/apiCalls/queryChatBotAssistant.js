...
export const queryChatBotAssistant = async (
  query,
  onMessage,
  onError,
  onComplete,
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SERVER_URI}/chatbot?query=${encodeURIComponent(
        query
      )}`,
      {
        method: "GET",
...
export const fetchCBARecentChats = async (user, chats, overrideChats) => {
  if (chats.length === 0 && user) {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URI}/chatbot/fetch-recent-chats?userId=${user.id}`,
        {
          method: "GET",
          headers: {
